...
def test_pairing_qr_script_can_force_refresh_valid_pair_code(tmp_path: Path):
    repo = tmp_path / "repo"
    scripts_dir = repo / "scripts"
    backend_dir = repo / "backend"
    fake_bin = tmp_path / "bin"
    ...
    pairing_path = backend_dir / "pairing.json"
    pairing_path.write_text(
        '{"server_url":"http://127.0.0.1:8000","server_urls":["http://127.0.0.1:8000"],"session_id":"iphone-app","pair_code":"valid-1234","pair_code_expires_at":"2999-01-01T00:00:00Z"}\n',
        encoding="utf-8",
    )
    ...
    assert result.returncode == 0, result.stderr
    updated = pairing_path.read_text(encoding="utf-8")
    assert '"pair_code": "valid-1234"' not in updated
    assert out_path.exists()
    assert out_path.read_bytes().startswith(b"\x89PNG\r\n\x1a\n")
