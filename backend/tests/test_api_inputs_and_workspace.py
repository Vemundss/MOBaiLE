...
def test_shell_utterance_flow(make_client):
...
def test_agent_utterance_flow_accepts_typed_attachments(make_client, tmp_path: Path):
...
def test_audio_mock_flow(make_client):
    ...
    resp = client.post(
        ...
        data={
            "session_id": "audio1",
            "executor": "shell",
            "transcript_hint": "python3 -c 'print(42)'",
        },
    )
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["transcript_text"] == "python3 -c 'print(42)'"
...
def test_audio_transcription_runs_outside_event_loop(make_client, monkeypatch):
    ...
    resp = client.post(
        ...
        headers=auth_headers(token),
        files={"audio": ("sample.wav", BytesIO(b"fakewav"), "audio/wav")},
        data={"session_id": "audio-threaded", "executor": "shell"},
    )
...
def test_audio_run_can_be_cancelled_during_transcription(make_client, monkeypatch):
    ...
    def post_audio() -> None:
        observed["response"] = client.post(
            ...
            headers=auth_headers(token),
            files={"audio": ("sample.wav", BytesIO(b"fakewav"), "audio/wav")},
            data={"session_id": "audio-cancel", "run_id": "audio-cancel", "executor": "shell"},
        )
...
def test_audio_openai_missing_key(make_client):
    ...
    resp = client.post(
        ...
        headers=auth_headers(token),
        files={"audio": ("sample.wav", BytesIO(b"fakewav"), "audio/wav")},
        data={"session_id": "audio2", "executor": "shell"},
    )
...
def test_audio_unexpected_transcription_failure_marks_precreated_run_failed(make_client, monkeypatch):
    ...
    resp = client.post(
        ...
        headers=auth_headers(token),
        files={"audio": ("sample.wav", BytesIO(b"fakewav"), "audio/wav")},
        data={"session_id": "audio-failed", "run_id": "audio-failed", "executor": "shell"},
    )
...
def test_audio_rejects_large_payload(make_client):
    ...
    resp = client.post(
        ...
        data={"session_id": "audio3", "run_id": "audio3", "executor": "shell"},
    )
