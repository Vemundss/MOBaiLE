...
class FakeExecutionService:
    ...
    def run_shell_command(
        ...
        self.calls.append(("shell", (run_id, command, workdir, guardrail_message)))
...
def test_utterance_service_routes_leading_bang_to_one_shot_shell(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path, **_agent_binary_env(tmp_path, "codex"))
    run_state = _run_state(tmp_path)
    execution = FakeExecutionService()
    launched: list[tuple[str, tuple[object, ...]]] = []
    ...
    service = UtteranceService(
        ...
        background_launcher=lambda target, args: launched.append((target.__name__, args)),
        run_id_factory=lambda: "run-shell-bang",
    )

    result = service.submit(
        UtteranceRequest(
            session_id="sess-shell-bang",
            utterance_text="!git status --short",
        )
    )

    assert result.status == "accepted"
    run = run_state.get_run("run-shell-bang")
    assert run is not None
    assert run.executor == "shell"
    assert run.utterance_text == "!git status --short"
    assert run.working_directory == str(project_dir)
    assert launched == [("run_shell_command", ("run-shell-bang", "git status --short", project_dir, None))]


def test_utterance_service_accepts_explicit_shell_executor(monkeypatch, tmp_path: Path) -> None:
    ...
    service = UtteranceService(
        ...
        background_launcher=lambda target, args: launched.append((target.__name__, args)),
        run_id_factory=lambda: "run-shell",
    )
    ...
    result = service.submit(
        UtteranceRequest(
            ...
            executor="shell",
            utterance_text="python3 -c 'print(42)'",
        )
    )

    assert result.status == "accepted"
    run = run_state.get_run("run-shell")
    assert run is not None
    assert run.executor == "shell"
    assert run.working_directory == str(project_dir)
    assert launched == [("run_shell_command", ("run-shell", "python3 -c 'print(42)'", project_dir, None))]


def test_utterance_service_persists_simple_shell_cd(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path)
    run_state = _run_state(tmp_path)
    execution = FakeExecutionService()
    launched: list[tuple[str, tuple[object, ...]]] = []
    ...
    service = UtteranceService(
        ...
        session_workdir_updater=lambda session_id, workdir: (
            updates.append((session_id, workdir))
            or _session_context(
                session_id=session_id,
                executor="shell",
                working_directory=workdir,
                resolved_working_directory=workdir,
            )
        ),
        background_launcher=lambda target, args: launched.append((target.__name__, args)),
        run_id_factory=lambda: "run-shell-cd",
    )

    result = service.submit(
        UtteranceRequest(
            session_id="sess-shell-cd",
            executor="shell",
            utterance_text="cd ios",
        )
    )

    assert result.status == "accepted"
    assert updates == [("sess-shell-cd", str(target_dir))]
    run = run_state.get_run("run-shell-cd")
    assert run is not None
    assert run.working_directory == str(project_dir)
    assert launched == [("run_shell_command", ("run-shell-cd", "cd ios", project_dir, None))]


def test_utterance_service_persists_leading_shell_cd_prefix(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path)
    run_state = _run_state(tmp_path)
    execution = FakeExecutionService()
    launched: list[tuple[str, tuple[object, ...]]] = []
    updates: list[tuple[str, str]] = []
    project_dir = env.default_workdir / "project"
    target_dir = project_dir / "ios app"
    command = 'cd "ios app" && pwd'
    target_dir.mkdir(parents=True)
    ...
    service = UtteranceService(
        ...
        background_launcher=lambda target, args: launched.append((target.__name__, args)),
        run_id_factory=lambda: "run-shell-cd-prefix",
    )

    result = service.submit(
        UtteranceRequest(
            session_id="sess-shell-cd-prefix",
            executor="shell",
            utterance_text=command,
        )
    )

    assert result.status == "accepted"
    assert updates == [("sess-shell-cd-prefix", str(target_dir))]
    run = run_state.get_run("run-shell-cd-prefix")
    assert run is not None
    assert run.working_directory == str(project_dir)
    assert launched == [("run_shell_command", ("run-shell-cd-prefix", command, project_dir, None))]


def test_utterance_service_treats_local_executor_as_shell_compatibility_alias(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path, VOICE_AGENT_DEFAULT_EXECUTOR="local")
    run_state = _run_state(tmp_path)
    execution = FakeExecutionService()
    launched: list[tuple[str, tuple[object, ...]]] = []
    ...
    service = UtteranceService(
        ...
        background_launcher=lambda target, args: launched.append((target.__name__, args)),
        run_id_factory=lambda: "run-local-compat",
    )

    result = service.submit(
        UtteranceRequest(
            session_id="sess-local-compat",
            utterance_text="pwd",
        )
    )

    assert result.status == "accepted"
    run = run_state.get_run("run-local-compat")
    assert run is not None
    assert run.executor == "shell"
    assert launched == [("run_shell_command", ("run-local-compat", "pwd", env.default_workdir, None))]


def test_utterance_service_rejects_empty_shell_escape(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path)
    run_state = _run_state(tmp_path)
    execution = FakeExecutionService()
    ...
    service = UtteranceService(
        ...
        background_launcher=lambda target, args: None,
        run_id_factory=lambda: "run-empty-shell",
    )

    result = service.submit(
        UtteranceRequest(
            session_id="sess-empty-shell",
            utterance_text="!   ",
        )
    )

    assert result.status == "rejected"
    run = run_state.get_run("run-empty-shell")
    assert run is not None
    assert run.executor == "shell"
    assert run.status == "rejected"
    assert run.summary == "Shell command is empty"
...
def test_utterance_service_marks_run_failed_when_background_launch_fails(monkeypatch, tmp_path: Path) -> None:
    ...
    result = service.submit(
        UtteranceRequest(
            ...
            utterance_text="python3 hello.py",
        )
    )
    ...
    assert run.summary == "Shell run failed to start"
...
def test_utterance_service_starts_new_request_without_cancelling_running_latest_run(monkeypatch, tmp_path: Path) -> None:
    env = _environment(monkeypatch, tmp_path, **_agent_binary_env(tmp_path, "codex"))
    run_state = _run_state(tmp_path)
    run_state.store_run(
        RunRecord(
    ...
    assert result.status == "accepted"
    old_run = run_state.get_run("run-old")
...
def test_utterance_service_falls_back_from_stale_session_executor(monkeypatch, tmp_path: Path) -> None:
    ...
    assert run is not None
    assert run.executor == "shell"
    assert launched[0][0] == "run_shell_command"
