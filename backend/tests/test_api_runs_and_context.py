...
def test_list_session_runs_and_diagnostics(make_client, monkeypatch):
    ...
    assert diagnostics["latest_activity"] == "Preparing the shell command result."
...
def test_session_context_updates_and_runs_inherit_defaults(make_client, tmp_path: Path):
    ...
    assert initial.json()["executor"] == "shell"
    ...
    run_payload = wait_for_run_to_settle(client, token, run_id, attempts=40)
    assert run_payload["executor"] == "shell"
    ...
    assert refreshed_payload["latest_run_status"] == "completed"
    assert refreshed_payload["latest_run_summary"] == "Command completed successfully"
...
def test_slash_command_catalog_and_execution(make_client, tmp_path: Path):
    ...
    assert payload[1]["usage"] == "/executor [shell]"
    assert payload[1]["argument_options"] == ["shell"]
    ...
    assert executor_payload["session_context"]["executor"] == "shell"
    assert "Available: shell." in executor_payload["message"]
