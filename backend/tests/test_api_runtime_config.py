...
def test_runtime_config_includes_codex_model(make_client, tmp_path: Path):
    ...
    assert "shell" in payload["available_executors"]
    ...
    assert executors["shell"]["kind"] == "shell"
    assert executors["shell"]["title"] == "Shell"
    assert executors["shell"]["internal_only"] is False
...
def test_config_endpoint_uses_shell_fallback_when_agent_binaries_are_missing(make_client, tmp_path: Path):
    ...
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["default_executor"] == "shell"
    assert payload["available_executors"] == ["shell"]
    executors = {item["id"]: item for item in payload["executors"]}
    assert executors["shell"]["default"] is True
