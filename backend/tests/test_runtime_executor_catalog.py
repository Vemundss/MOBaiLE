...
def test_build_runtime_executor_descriptors_marks_defaults_and_models() -> None:
    ...
    assert descriptors["shell"].kind == "shell"
    assert descriptors["shell"].available is True
    assert descriptors["shell"].internal_only is False
...
def test_build_runtime_config_response_projects_runtime_executor_state() -> None:
    ...
    response = build_runtime_config_response(
        env,
        available_executors=["shell", "codex", "claude"],
    ...
    assert response.available_executors == ["shell", "codex", "claude"]
