...
def test_refresh_pairing_server_url_keeps_previous_tailscale_when_detection_is_lan_only(
    monkeypatch,
    tmp_path: Path,
):
    module = importlib.import_module("app.pairing_url")
    ...
    monkeypatch.setattr(
        module,
        "detect_server_urls",
        lambda **_: ["http://192.168.1.20:8000"],
    )
...
def test_refresh_pairing_server_url_discards_stale_public_url_without_override(monkeypatch, tmp_path: Path):
    module = importlib.import_module("app.pairing_url")
    module = importlib.reload(module)
    monkeypatch.setattr(
        module,
...
def test_refresh_pairing_server_url_switches_from_stale_public_to_wifi(monkeypatch, tmp_path: Path):
    module = importlib.import_module("app.pairing_url")
    module = importlib.reload(module)
    monkeypatch.setattr(module, "detect_tailscale_dns_name", lambda: "mobaile.tail6a5903.ts.net")
    monkeypatch.setattr(module, "detect_tailscale_ip", lambda: "100.111.99.51")
...
def test_refresh_pairing_server_url_switches_from_stale_public_to_local(monkeypatch, tmp_path: Path):
    module = importlib.import_module("app.pairing_url")
    module = importlib.reload(module)
    monkeypatch.setattr(module, "detect_tailscale_dns_name", lambda: "mobaile.tail6a5903.ts.net")
    monkeypatch.setattr(module, "detect_tailscale_ip", lambda: "100.111.99.51")
...
def test_refresh_pairing_server_url_preserves_previous_tailscale_url_when_detection_degrades(monkeypatch, tmp_path: Path):
    module = importlib.import_module("app.pairing_url")
    module = importlib.reload(module)
    monkeypatch.setattr(
        module,
        "detect_server_urls",
        lambda **_: ["http://127.0.0.1:8000"],
    )
...
def test_refresh_pairing_server_url_preserves_previous_wifi_url_when_detection_degrades(monkeypatch, tmp_path: Path):
    module = importlib.import_module("app.pairing_url")
    module = importlib.reload(module)
    monkeypatch.setattr(
        module,
