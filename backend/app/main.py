...
from app.onboarding_events import OnboardingEventRecorder
...
from app.setup_readiness import (
...
UTTERANCE_SERVICE = UtteranceService(
    ...
    session_workdir_updater=RUNTIME_SESSION_SERVICE.update_working_directory,
)
WORKSPACE_SERVICE = WorkspaceService(ENV)
ONBOARDING_EVENTS = OnboardingEventRecorder(ENV.backend_root / "data" / "onboarding-events.jsonl")
CREDENTIAL_REQUEST_SERVICE = CredentialRequestService(ENV.db_path.parent / "credential-requests.json")
...
def setup_page(request: Request) -> HTMLResponse:
    ...
    ONBOARDING_EVENTS.record("setup_page_opened", {"surface": "browser"})
    return HTMLResponse(setup_page_html())


@app.get("/setup/readiness", response_model=SetupReadinessResponse)
def get_local_setup_readiness(request: Request) -> SetupReadinessResponse:
    _require_local_setup_request(request)
    readiness = _setup_readiness(include_local_setup_urls=True)
    ONBOARDING_EVENTS.record(
        "setup_readiness_checked",
        {"surface": "browser", "status": readiness.status},
    )
    return readiness


@app.get("/setup/pairing-qr.png", response_class=FileResponse)
def get_local_setup_pairing_qr(request: Request) -> FileResponse:
    _require_local_setup_request(request)
    qr_path = ENV.pairing_file.with_name("pairing-qr.png")
    if not qr_path.exists():
        raise HTTPException(status_code=404, detail="pairing QR not found; run mobaile pair")
    ONBOARDING_EVENTS.record("pairing_qr_viewed", {"surface": "browser"})
    return FileResponse(str(qr_path), media_type="image/png")


@app.get("/setup/onboarding-report.json")
def get_local_onboarding_report(request: Request) -> dict[str, object]:
    _require_local_setup_request(request)
    return _onboarding_report()


@app.get("/v1/setup/readiness", response_model=SetupReadinessResponse)
def get_setup_readiness() -> SetupReadinessResponse:
    readiness = _setup_readiness(include_local_setup_urls=False)
    ONBOARDING_EVENTS.record(
        "setup_readiness_checked",
        {"surface": "api", "status": readiness.status},
    )
    return readiness


@app.get("/v1/setup/onboarding-report")
def get_onboarding_report() -> dict[str, object]:
    return _onboarding_report()
...
def _setup_readiness(*, include_local_setup_urls: bool) -> SetupReadinessResponse:
    return build_setup_readiness(
        ENV,
        PAIRING_SERVICE,
        RUN_STATE,
...
def _onboarding_report() -> dict[str, object]:
    report = ONBOARDING_EVENTS.report()
    first_run = build_setup_readiness(
        ENV,
        PAIRING_SERVICE,
        RUN_STATE,
        include_local_setup_urls=False,
    ).first_run
    report["first_run"] = first_run.model_dump()
    return report


def _require_local_setup_request(request: Request) -> None:
    _require_local_request(request, detail="setup page is only available from this computer")


def _require_local_request(request: Request, *, detail: str) -> None:
    client_host = (request.client.host if request.client else "").strip().lower()
    if client_host in {"testclient", "localhost", "::1", "[::1]"} or client_host.startswith("127."):
        return
...
def pair_exchange(payload: PairExchangeRequest, request: Request) -> PairExchangeResponse:
    response = PAIRING_SERVICE.exchange_pair_code(
        payload,
    ...
    ONBOARDING_EVENTS.record(
        "pairing_success",
        {
            ...
            "server_url_count": len(response.server_urls),
        },
    )
    return response
...
def create_utterance(request: UtteranceRequest) -> UtteranceResponse:
    ...
    response = UTTERANCE_SERVICE.submit(request)
    if request.session_id == "mobaile-first-run":
        ONBOARDING_EVENTS.record(
            "first_run_started",
            {
                "run_id": response.run_id,
                "status": response.status,
                "executor": request.executor or ENV.default_executor,
            },
        )
    return response
...
@app.post("/v1/credential-requests", response_model=CredentialRequestRecord)
def create_credential_request(payload: CredentialRequestCreateRequest) -> CredentialRequestRecord:
    return CREDENTIAL_REQUEST_SERVICE.create(payload)


@app.get("/v1/credential-requests", response_model=CredentialRequestListResponse)
def list_credential_requests(
    session_id: str | None = Query(None, min_length=1),
    status: Literal["pending", "fulfilled", "cancelled", "expired"] | None = None,
) -> CredentialRequestListResponse:
    return CredentialRequestListResponse(
        requests=CREDENTIAL_REQUEST_SERVICE.list(session_id=session_id, status=status)
    )


@app.get("/v1/credential-requests/{request_id}", response_model=CredentialRequestRecord)
def get_credential_request(request_id: str) -> CredentialRequestRecord:
    try:
        return CREDENTIAL_REQUEST_SERVICE.get(request_id)
    except CredentialRequestNotFoundError as exc:
        raise HTTPException(status_code=404, detail="credential request not found") from exc


@app.post("/v1/credential-requests/{request_id}/fulfill", response_model=CredentialRequestFulfillResponse)
def fulfill_credential_request(
    request_id: str,
    payload: CredentialRequestFulfillRequest,
) -> CredentialRequestFulfillResponse:
    try:
        return CREDENTIAL_REQUEST_SERVICE.fulfill(request_id, payload)
    except CredentialRequestNotFoundError as exc:
        raise HTTPException(status_code=404, detail="credential request not found") from exc
    except CredentialRequestConflictError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    except CredentialRequestValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@app.post("/v1/credential-requests/{request_id}/resolve", response_model=CredentialRequestResolveResponse)
def resolve_credential_request(
    request_id: str,
    request: Request,
    consume: bool = Query(True),
) -> CredentialRequestResolveResponse:
    _require_local_request(
        request,
        detail="credential resolution is only available from this computer",
    )
    try:
        return CREDENTIAL_REQUEST_SERVICE.resolve(request_id, consume=consume)
    except CredentialRequestNotFoundError as exc:
        raise HTTPException(status_code=404, detail="credential request not found") from exc
    except CredentialRequestConflictError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
@app.post("/v1/credential-requests/{request_id}/cancel", response_model=CredentialRequestRecord)
def cancel_credential_request(request_id: str) -> CredentialRequestRecord:
    try:
        return CREDENTIAL_REQUEST_SERVICE.cancel(request_id)
    except CredentialRequestNotFoundError as exc:
        raise HTTPException(status_code=404, detail="credential request not found") from exc
    except CredentialRequestConflictError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
