...
if TYPE_CHECKING:
    from app.models.schemas import AgentExecutorName, RunExecutorName
...
def build_runtime_executor_descriptors(
    ...
    *,
    ...
    return [
        RuntimeExecutorDescriptor(
            id="shell",
            title=EXECUTOR_TITLES["shell"],
            kind="shell",
            available=True,
            default=env.default_executor == "shell",
            internal_only=False,
        ),
        RuntimeExecutorDescriptor(
            id="codex",
            title=EXECUTOR_TITLES["codex"],
            kind="agent",
...
def build_runtime_config_response(
    env: "RuntimeEnvironment",
    *,
    available_executors: list["RunExecutorName"],
    ...
    return RuntimeConfigResponse(
        ...
        executors=build_runtime_executor_descriptors(
            env,
            available_agents={executor for executor in available_executors if executor in {"codex", "claude"}},
        ),
