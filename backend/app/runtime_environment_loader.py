...
def configured_default_executor() -> RunExecutorName:
    ...
    if configured == "local":
        configured = "shell"
    if configured not in {"shell", "codex", "claude"}:
        configured = "codex"
...
def resolve_default_executor(
    ...
    if configured_executor in {"shell", "local"}:
        return "shell"
...
def load_agent_runtime_environment_settings(backend_root: Path) -> AgentRuntimeEnvironmentSettings:
    shell_binary = _default_shell_binary()
    shell_timeout_sec = _read_non_negative_int_env("VOICE_AGENT_SHELL_TIMEOUT_SEC", 120)
...
def _default_shell_binary() -> str:
    configured = os.getenv("VOICE_AGENT_SHELL_BINARY", "").strip()
    if configured:
        return configured
    login_shell = os.getenv("SHELL", "").strip()
    if login_shell:
        return login_shell
