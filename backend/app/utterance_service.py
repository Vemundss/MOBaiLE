...
class UtteranceService:
    def __init__(
        ...
        *,
        ...
        session_workdir_updater: Callable[[str, str], SessionContextResponse] | None = None,
        ...
        self.session_workdir_updater = session_workdir_updater
    ...
    def _submit_prepared(
        ...
        *,
        ...
        if prepared.executor == "shell":
            return self._submit_shell_request(prepared, precreated=precreated)
        ...
        raise HTTPException(status_code=409, detail=f"executor {prepared.executor} is not available on this backend")
    ...
    def _prepare(self, request: UtteranceRequest, *, run_id: str | None = None) -> PreparedUtterance:
        ...
        shell_command = self._one_shot_shell_command(request.utterance_text)
        if shell_command is not None:
            executor = "shell"
            display_text = f"!{shell_command}" if shell_command else "!"
            effective_text = shell_command
        elif executor == "shell":
            shell_command = self._strip_optional_shell_escape(effective_text)
            effective_text = shell_command
    ...
    def _submit_shell_request(self, prepared: PreparedUtterance, *, precreated: bool) -> UtteranceResponse:
        command = (prepared.shell_command or prepared.effective_text).strip()
        if not command:
            self._record_rejected_run(
                prepared,
            ...
            return self._rejected_response(prepared.run_id, "Shell command is empty")

        guardrail_status, guardrail_message = self.environment.evaluate_runtime_guardrails(command)
        if guardrail_status == "reject":
            self._record_rejected_run(
                prepared,
                summary=guardrail_message,
        ...
        if self.session_workdir_updater is not None:
            try:
                next_workdir = persistent_cd_target(
                    command,
                    prepared.workdir,
                    workdir_root=self.environment.workdir_root,
                )
            except ValueError as exc:
                message = str(exc)
                self._record_rejected_run(
            ...
            if next_workdir is not None:
                try:
                    self.session_workdir_updater(prepared.session_context.session_id, str(next_workdir))
                    ...
                    message = "Working directory update failed"
                    self._record_rejected_run(
                        prepared,
                        summary=message,
                        message=message,
        ...
        if not self._launch_background_run(
            prepared.run_id,
            self.execution_service.run_shell_command,
            (
                prepared.run_id,
                command,
                prepared.workdir,
            ...
            failure_summary="Shell run failed to start",
        ):
            return self._rejected_response(prepared.run_id, "Shell run failed to start")
        return self._accepted_response(prepared.run_id)
    ...
    @staticmethod
    def _running_run_record(prepared: PreparedUtterance) -> RunRecord:
        return RunRecord(
            run_id=prepared.run_id,
            session_id=prepared.session_context.session_id,
            executor=prepared.executor,
    ...
    @staticmethod
    def _one_shot_shell_command(utterance_text: str) -> str | None:
        trimmed = utterance_text.lstrip()
        if not trimmed.startswith("!"):
            return None
        return trimmed[1:].strip()

    @staticmethod
    def _strip_optional_shell_escape(command: str) -> str:
