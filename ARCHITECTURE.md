...
1. `shell`
- Direct shell executor for explicit host commands and smoke checks.
- Backend runs the command in the requested working directory using the configured user shell.
- In normal agent mode, a leading `!` routes that single prompt to shell, for example `!git status`.
...
- SwiftUI client in the private sibling frontend repository
