# MOBaiLE Backend Repo Agents

This file is the backend repo contract for human contributors and coding agents working on MOBaiLE.
...
This repo contains the public backend, installer, runtime policy, and generated API contracts. The private iOS
frontend lives in the sibling `../frontend` repository when working from the local wrapper workspace.

## Scope

- Use this file for development guidance only.
- Read `README.md`, `backend/README.md`, `docs/USAGE.md`, and `ARCHITECTURE.md` for product direction and current backend shape.
...
- For cross-repo work, launch from the wrapper directory and read `../AGENTS.md` first.
...
- `scripts/`: backend installer, service management, doctor, pairing, and contract sync scripts.
- `docs/`: public backend/product docs that are safe to ship with the backend repository.
...
- Update `docs/USAGE.md`, `backend/README.md`, `scripts/README.md`, or `ARCHITECTURE.md` when the touched area changes their assumptions.
