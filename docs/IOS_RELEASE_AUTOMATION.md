...
```bash
npm run ios:release:prepare -- version:1.0.1
```
...
```bash
npm run ios:release:testflight -- version:1.0.1 changelog:"Bug fixes and polish"
```

Build and upload an App Store Connect release build without metadata/screenshots:

```bash
npm run ios:release:appstore -- version:1.0.1
```

Upload and immediately submit for review:

```bash
npm run ios:release:appstore -- version:1.0.1 submit_for_review:true automatic_release:false
```
