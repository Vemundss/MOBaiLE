...
main() {
  ...
  if [[ "${has_codex}" == "true" || "${has_claude}" == "true" ]]; then
    ...
    echo "No Codex/Claude CLI detected. MOBaiLE can still run direct shell commands from the paired host."
    echo "Install and sign in to Codex CLI for real coding runs: https://developers.openai.com/codex/cli"
  fi
  echo
