...
Options:
  ...
  --yes                        Skip prompts and use the recommended defaults
...
codex_available() {
  ...
  if [[ "$(uname -s)" == "Darwin" && -x "/Applications/Codex.app/Contents/Resources/codex" ]]; then
    return 0
  fi
  return 1
}
...
agent_cli_available() {
  codex_available || command -v claude > /dev/null 2>&1
}
...
autonomy_label() {
  if [[ "${MODE}" == "full-access" ]]; then
    ...
    printf "Limited by safe mode\n"
  fi
}

should_run_autonomy_setup() {
  [[ "${MODE}" == "full-access" ]]
}
...
print_dependency_notes() {
  ...
  if [[ "${PHONE_ACCESS_MODE}" == "tailscale" && -z "${PUBLIC_SERVER_URL}" ]]; then
    if command -v tailscale > /dev/null 2>&1; then
      ...
      echo "  Tailscale: needed for the default Anywhere mode. Install it on this computer and the iPhone, then sign in to the same tailnet."
      echo "    Download: https://tailscale.com/download"
    fi
  elif [[ "${PHONE_ACCESS_MODE}" == "wifi" ]]; then
    echo "  Tailscale: not required for Wi-Fi-only pairing, but cellular/off-network use needs Tailscale or a public HTTPS URL."
  elif [[ "${PHONE_ACCESS_MODE}" == "local" ]]; then
    echo "  Tailscale: not used in local-only mode."
  elif [[ -n "${PUBLIC_SERVER_URL}" ]]; then
    echo "  Tailscale: not required when the phone uses the configured public HTTPS URL."
  fi
}
...
print_product_summary() {
  ...
  echo "Autonomy setup: $(autonomy_label)"
  if [[ -n "${PUBLIC_SERVER_URL}" ]]; then
    echo "Public URL: ${PUBLIC_SERVER_URL}"
  fi
  echo
  echo "QR image: ${qr_path}"
  echo "Setup page: run \`mobaile setup\` on this computer for the computer-local checklist."
  print_dependency_notes
  echo
  echo "Next:"
